...
author: "Bent Eelman: 2713151, Twan van Neijenhof: 2696735, Julius Tempelaars: 2699775. Team: 13"
...
question1b<-function(){
  ...
  d$dummy <- ifelse(d$roomtype=="Private room", 1, 0)
  table(d$dummy)
...
question2a<-function(){
  ...
  # first create the dummy variable using ifelse() statements
   ...
   d$onebedroom <- ifelse(d$roomtype == "Entire home/apt" & d$bedrooms == 1, "Entire home/apt", NA)
   d$onebedroom <- ifelse(d$roomtype == "Private room" & d$bedrooms == 1, "Private room", d$onebedroom)
   d$onebedroom <<- d$onebedroom
...
question3<-function(){
  ...
  pricelast2 <- d$pricelast[which(d$pricelast <= 500)]
  #test to see if it doesn't show > 500
  pricelast2
  
  review.p.m <- d$reviewspermonth[which(d$pricelast <= 500)]
  
  print(cor(review.p.m, pricelast2, use = "complete.obs"))
  
  #print maximum value of reviews per month among all listings
  
  print(max(d$reviewspermonth, na.rm = TRUE))
  
  #create a scatterplot of this relationship for listings with a price below 500$
  
  plot(pricelast2, review.p.m)
...
question4b<-function(){
  ...
  # first calculate the average when moving average is 1
    print(mean(d$review_scores_rating[na.ma(1)]))
  
  # then calculate the average when moving average is 10
    print(mean(d$review_scores_rating[na.ma(10)]))
...
question5a<-function(){
  ...
  review_scores_averages <- rowMeans(d[, 22:27], na.rm = TRUE)

  review_scores_averages[is.na(review_scores_averages)] <- 0
  review_scores_averages <- ifelse(review_scores_averages <= 4.94, "lower", "higher")
  ...
  d$review_scores_averages <<- review_scores_averages
  
  # run table last so it is shown in your output below
  table(d$review_scores_averages)
...
question5b<-function(){
   ...
   ggplot1 <- ggplot(data = remove_missing(d, na.rm = TRUE, vars = d$review_scores_averages), mapping = aes(x =  availability_90, y = pricelast, col = review_scores_averages)) + geom_point() + geom_smooth( method = "lm") + labs(x = "Availability over last quarter", y = "Last known price of rent for property" ) + ylim(3, 999) 
  
  
  print(ggplot(data = remove_missing(d, na.rm = TRUE, vars = d$review_scores_averages), mapping = aes(x =  availability_90, y = pricelast, col = review_scores_averages)) + geom_point() + geom_smooth( method = "lm") + labs(x = "Availability over last quarter", y = "Last known price of rent for property" ) + ylim(3, 999)) 
  
  
  
  ggplot2 <- ggplot(data = remove_missing(d, na.rm = TRUE, vars = d$review_scores_averages), mapping = aes(x =  availability_90, y = pricelast, col = review_scores_averages)) + geom_point() + geom_smooth( method = "lm") + labs(x = "Availability over last quarter", y = "Last known price of rent for property" ) + ylim(3, 499)

print(ggplot(data = remove_missing(d, na.rm = TRUE, vars = d$review_scores_averages), mapping = aes(x =  availability_90, y = pricelast, col = review_scores_averages)) + geom_point() + geom_smooth( method = "lm") + labs(x = "Availability over last quarter", y = "Last known price of rent for property" ) + ylim(3, 499))
...
question5c<-function(){
 ...
 # write your code here
  ...
  sort(d$pricelast)
     ...
     print(Price_grp <-cut(d$pricelast, breaks = c(1, 83, 150, 3000)))
     print(d$price_grp1 <- Price_grp)
  # when the grouping variable is created, use <<- to save in your environment
  
  # ANSWER1: run table of grouping variable to check the distribution
     print(table(Price_grp))
  
  # ANSWER2: print the average price per group by using tapply 
     tapply(d$pricelast, d$price_grp1, mean)
}
question5c()
```
...
question5d<-function(){
  ...
  # write your code here
    print(tapply(d$pricelast[d$price_grp1 == "(100,3000]"], d$availability_90[d$price_grp1 == "(100,3000]"]))
  
  # run an anova to test whether the three price groups differ in availability_90
  print(aov_results <- aov(d$availability_90 ~ d$pricelast[d&price_grp1]))
  
  # ANSWER1: summary of the anova output
  summary(aov_results)
...
question5e<-function(){
 ...
 # write your code here
  ...
  print(availability_70 <- ifelse(d$availability_90 >= 70, "More", "Less"))
 
  
  # you have to run the following line (remove the hashtag & change names):
  # dataframe$dummy <<- dataframe$dummy 
  
  print(availability_70 <<- availability_70)
  
  # ANSWER1: use table to inspect how many listings were available more than 70 days
   
  table(availability_70)
}
question5e()
```
